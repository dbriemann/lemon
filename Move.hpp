...
static const U64 CASTLE_SHORT_PATH[2] = {0x0000000000000060, 0x6000000000000000};
static const U64 CASTLE_LONG_PATH[2] = {0x000000000000000E, 0x0E00000000000000};
static const U64 CASTLE_LONG_CHECK_PATH[2] = {0x000000000000000C, 0x0C00000000000000};
...
Move moveCreate(U32 from, U32 to, U32 ptype, U32 capflag, U32 promtype, U32 eptype, U32 castleflag, U32 value) {
    Move m = from;
    to <<= TO_SHIFT;
    ptype <<= PIECE_SHIFT;    
...
//__attribute__ ((noinline))
void moveSetFeature(Move &m, const U32 mask, const U32 shift, U32 bits) {
    m &= ~mask;
    bits <<= shift;
    m |= bits;
}
...
//__attribute__ ((noinline))
U32 moveGetFeature(const Move &m, const U32 mask, const U32 shift) {
    return (m & mask) >> shift;
}
